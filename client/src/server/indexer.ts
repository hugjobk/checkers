...
export const createIndexer = async () => {
  ...
  app.patch("/games/:gameId", async (req: Request, res: Response) => {
    let found: boolean;
    try {
      found = await patchGame(req.params.gameId);
    } catch (error: any) {
      res.status(500).json({
  ...
  const patchGame = async (gameId: string): Promise<boolean> => {
    let game: StoredGame | undefined;
    try {
      game = await client.checkersQueryClient?.checkers.getStoredGame(gameId);
    } catch (error: any) {
  ...
  const poll = async () => {
    ...
    while (db.status.block.height < currentHeight) {
      let processing = db.status.block.height + 1;
      process.stdout.cursorTo(0);
      let block: Block;
      while (true) {
        try {
          block = await client.getBlock(processing);
        } catch (error: any) {
          // Get the lowest available block
          const err = JSON.parse(error.message);
          // Unhandled error
          if (err.code != -32603) throw error;
          const matches = /lowest\sheight\sis\s(\d+)/.exec(err.data);
          const lowestBlock = parseInt(matches![1]);
          console.log(`Skipping to lowest available block: ${lowestBlock}`);
          processing = lowestBlock;
          continue;
        }
        break;
      }
